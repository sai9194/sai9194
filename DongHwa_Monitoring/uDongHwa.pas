...
uses
  ...
  System.PushNotification,  System.JSON,  // for Pus
...
type
  ...
  private
    ...
    function IsRealPhysicalKeyboard: Boolean;
  public
...
// =============================================================================
// ★ [여기서부터 추가] Windows API 직접 정의 및 물리 키보드 감지 로직
// =============================================================================
const
...
// user32.dll에서 함수 직접 불러오기 (Delphi 버전에 상관없이 동작하도록)
function GetRawInputDeviceList(pRawInputDeviceList: PRAWINPUTDEVICELIST;
  var puiNumDevices: UINT; cbSize: UINT): UINT; stdcall; external 'user32.dll';

function GetRawInputDeviceInfoW(hDevice: THandle; uiCommand: UINT;
  pData: Pointer; var pcbSize: UINT): UINT; stdcall; external 'user32.dll';

// 물리 키보드 판별 함수 구현
function TfDongHwa.IsRealPhysicalKeyboard: Boolean;
var
  DeviceCount, I: UINT;
...
begin
  ...
  // 1. 장치 개수 확인
  if GetRawInputDeviceList(nil, DeviceCount, SizeOf(RAWINPUTDEVICELIST)) <> 0 then Exit;
  if DeviceCount = 0 then Exit;

  // 2. 메모리 할당 및 데이터 가져오기
  SetLength(DeviceList, DeviceCount);
  if GetRawInputDeviceList(@DeviceList[0], DeviceCount, SizeOf(RAWINPUTDEVICELIST)) = UINT(-1) then Exit;

  // 3. 키보드 검색 및 이름 필터링
  for I := 0 to DeviceCount - 1 do
  begin
    if DeviceList[I].dwType = RIM_TYPEKEYBOARD then
    begin
      NameLen := 0;
      GetRawInputDeviceInfoW(DeviceList[I].hDevice, RIDI_DEVICENAME, nil, NameLen);

      if NameLen > 0 then
      begin
        SetLength(Buffer, NameLen + 1);
        if GetRawInputDeviceInfoW(DeviceList[I].hDevice, RIDI_DEVICENAME, @Buffer[0], NameLen) > 0 then
        begin
          DeviceName := UpperCase(String(PChar(@Buffer[0])));

          // 가상 키보드나 RDP 등은 제외하고, HID 또는 USB 키보드만 진짜로 인정
          if (Pos('HID', DeviceName) > 0) or (Pos('USB', DeviceName) > 0) then
          begin
             // 바코드 스캐너도 HID로 잡히지만, 보통 키보드와 동일하게 취급해도 무방함
             Result := True;
             Break;
          end;
        end;
...
begin
  try
    ...
    if ControllerID[SenderTag] = 0 then exit;
...
//    // 240828 김동휘추가
    ...
    begin
     ...
     // ParamByName('Name').AsString := TextDetailTitle.Text;
      ...
      Text_Title2.Text := '등록일 : ' +FieldByName('ModifiedDateTime').AsString;
    end;
...
begin
  try
    ...
    WITH UniQueryDetailSave do
    begin
      Close;
      SQL.Clear;
      SQL.Add('Update ControllerInfo');
      SQL.Add('Set ControllerName = '+#39+EditDetailName.Text+#39);
      SQL.Add(', TempName = '+#39+EditDetailTemp.Text+#39);
      SQL.Add(', SetTempName = '+#39+EditDetailSetTemp.Text+#39);
      SQL.Add(', In1 = '+#39+EditDetailIn1.Text+#39);
      SQL.Add(', In2 = '+#39+EditDetailIn2.Text+#39);
      SQL.Add(', In3 = '+#39+EditDetailIn3.Text+#39);
      SQL.Add(', In4 = '+#39+EditDetailIn4.Text+#39);
      SQL.Add(', In5 = '+#39+EditDetailIn5.Text+#39);
      SQL.Add(', Out1 = '+#39+EditDetailOut1.Text+#39);
      SQL.Add(', Out2 = '+#39+EditDetailOut2.Text+#39);
      SQL.Add(', Out3 = '+#39+EditDetailOut3.Text+#39);
      SQL.Add(', Out4 = '+#39+EditDetailOut4.Text+#39);
      SQL.Add(', Out5 = '+#39+EditDetailOut5.Text+#39);
      SQL.Add(', HighTemp = '+#39+ FloatToStrF(hightemp, ffFixed, 7, 1)+#39);
      SQL.Add(', LowTemp = '+#39+FloatToStrF(Lowtemp, ffFixed, 7, 1)+#39);
      SQL.Add('Where ControllerID = '+DetailControllerID.ToString);
      ExecSQL;
    end;
...
if IsRealPhysicalKeyboard then
    VKAutoShowMode := TVKAutoShowMode.DefinedBySystem
  else
...
begin
  ...
  Chart1.LeftAxis.Title.Text := '온도'+#13#10+'(℃)';
...
begin
  try
    ...
    begin
      try
        with  UniQueryLogin do
        begin
          Close;
          SQL.Clear;
          SQL.Add('Exec usp_User @Option = ''O'', @LoginID = :LoginID');
          ParamByName('LoginID').AsString := EditJoinID.Text;
          ExecSQL;
          if RecordCount > 0 then
          begin
          ...
          begin
            Close;
            SQL.Clear;
            SQL.Add('Exec usp_User @Option = ''E'', @LoginID = :LoginID, @Password = :Password ');
            SQL.Add(', @Name = :Name, @Position = :Position ');
            ParamByName('LoginID').AsString := EditJoinID.Text;
            ParamByName('Password').AsString := EditJoinPW.Text;
            ParamByName('Name').AsString := EditJoinCompany.Text;
            ParamByName('Position').AsString := '관리자';
            ExecSQL;
...
//        SQL.Add('And A.EndTime IS NULL');
        ...
        begin
          ...
          TextMain1SetTemp.TextSettings.Font.Size := PCerrSetSize;
...
//                end;
          ...
          begin
            ...
            TextMain2SetTemp.TextSettings.Font.Size := PCerrSetSize;
            if FieldByName('PowerState').AsBoolean and ((Now - (0.0000011574 * 600 * 5)) <= FieldByName('CheckDate').AsDateTime) then
            begin
...
//              TextMain2Temp.TextSettings.Font.Size := TitleSize;
          ...
          begin
            ...
            TextMain3SetTemp.TextSettings.Font.Size := PCerrSetSize;
            if FieldByName('PowerState').AsBoolean and ((Now - (0.0000011574 * 600 * 5)) <= FieldByName('CheckDate').AsDateTime) then
            begin
...
//              TextMain3Temp.TextSettings.Font.Size := TitleSize;
          ...
          begin
            ...
            TextMain4SetTemp.TextSettings.Font.Size := PCerrSetSize;
            if FieldByName('PowerState').AsBoolean and ((Now - (0.0000011574 * 600 * 5)) <= FieldByName('CheckDate').AsDateTime) then
            begin
...
//              TextMain4Temp.TextSettings.Font.Size := TitleSize;
          ...
          begin
            ...
            TextMain5SetTemp.TextSettings.Font.Size := PCerrSetSize;
            if FieldByName('PowerState').AsBoolean and ((Now - (0.0000011574 * 600 * 5)) <= FieldByName('CheckDate').AsDateTime) then
            begin
...
//              Textmain5SetTemp.TextSettings.Font.Size := PCTempSetSize;
          ...
          begin
            ...
            TextMain6SetTemp.TextSettings.Font.Size := PCerrSetSize;
            if FieldByName('PowerState').AsBoolean and ((Now - (0.0000011574 * 600 * 5)) <= FieldByName('CheckDate').AsDateTime) then
            begin
...
//              Textmain6SetTemp.TextSettings.Font.Size := PCTempSetSize;
          ...
          begin
            ...
            TextMain7SetTemp.TextSettings.Font.Size := PCerrSetSize;
            if FieldByName('PowerState').AsBoolean and ((Now - (0.0000011574 * 600 * 5)) <= FieldByName('CheckDate').AsDateTime) then
            begin
...
//              Textmain7SetTemp.TextSettings.Font.Size := PCTempSetSize;
          ...
          begin
            ...
            DefrostState[Index] := FieldByName('DefrostState').AsBoolean;
             ...
             TextMain8SetTemp.TextSettings.Font.Size := PCerrSetSize;
            if FieldByName('PowerState').AsBoolean and ((Now - (0.0000011574 * 600 * 5)) <= FieldByName('CheckDate').AsDateTime) then
            begin
...
//              Textmain8SetTemp.TextSettings.Font.Size := PCTempSetSize;
          ...
          begin
            ...
            TextMain9SetTemp.TextSettings.Font.Size := PCerrSetSize;
            if FieldByName('PowerState').AsBoolean and ((Now - (0.0000011574 * 600 * 5)) <= FieldByName('CheckDate').AsDateTime) then
            begin
...
//              Textmain9SetTemp.TextSettings.Font.Size := PCTempSetSize;
          ...
          begin
            ...
            TextMain10SetTemp.TextSettings.Font.Size := PCerrSetSize;
            if FieldByName('PowerState').AsBoolean and ((Now - (0.0000011574 * 600 * 5)) <= FieldByName('CheckDate').AsDateTime) then
            begin
...
//              Textmain10SetTemp.TextSettings.Font.Size := PCTempSetSize;
          ...
          begin
            ...
            TextMain11SetTemp.TextSettings.Font.Size := PCerrSetSize;
            if FieldByName('PowerState').AsBoolean and ((Now - (0.0000011574 * 600 * 5)) <= FieldByName('CheckDate').AsDateTime) then
            begin
...
//              Textmain11SetTemp.TextSettings.Font.Size := PCTempSetSize;
          ...
          begin
            ...
            TextMain12SetTemp.TextSettings.Font.Size := PCerrSetSize;
            if FieldByName('PowerState').AsBoolean and ((Now - (0.0000011574 * 600 * 5)) <= FieldByName('CheckDate').AsDateTime) then
            begin
...
begin
  ...
  {$IFDEF MSWINDOWS}
    ...
    PCTempSize    := Round(ClientWidthDiv3 / 12);
    PCTempSetSize := Round(ClientWidthDiv3 / 12);
...
//   왼쪽 하단
    ...
    begin
      close;
      sql.Clear;
      // T.CheckDate와 A.StartTime이 가장 가까운 레코드 매칭
      sql.add(' WITH ClosestTemp AS (																											');
      sql.add('     SELECT A.[No] as AlarmNo, A.[ErrCode], A.[StartTime], A.[EndTime], A.[CheckTime], A.[Dcheck],								');
      sql.add('            T.[No], T.[CheckDate], T.[SerialNo], T.[Temp], T.[SetTemp], T.[PowerState],										');
      sql.add('            T.[DIn1], T.[DIn2], T.[DIn3], T.[DIn4], T.[DIn5],																	');
      sql.add('            T.[DOut1], T.[DOut2], T.[DOut3], T.[DOut4], T.[DOut5],																');
      sql.add('            T.[Err], T.[DefrostMode], T.[DefrostState],																		');
      sql.add('            ROW_NUMBER() OVER (PARTITION BY A.[No] ORDER BY ABS(DATEDIFF(SECOND, T.CheckDate, A.StartTime))) as rn				');
      sql.add('     FROM [dbo].[Alarm] A WITH(NOLOCK)																							');
      sql.add('     LEFT JOIN [dbo].[TempCheck] T WITH(NOLOCK) ON T.SerialNo IN (																');
      sql.add('         SELECT SerialNo FROM ControllerInfo WHERE ControllerID = '+DetailControllerID.ToString);
      sql.add('     )																															');
      sql.add('     WHERE A.ControllerID = '+DetailControllerID.ToString);
      sql.add(' )																																');
      sql.add(' SELECT T.[No], T.[CheckDate], T.[SerialNo], T.[Temp], T.[SetTemp], T.[PowerState],											');
      sql.add('        T.[DIn1], T.[DIn2], T.[DIn3], T.[DIn4], T.[DIn5],																		');
      sql.add('        T.[DOut1], T.[DOut2], T.[DOut3], T.[DOut4], T.[DOut5],																	');
      sql.add('        T.[Err], T.[DefrostMode], T.[DefrostState],																			');
      sql.add('        CT.AlarmNo, CT.[ErrCode], CT.[StartTime], CT.[EndTime], CT.[CheckTime], CT.[Dcheck]									');
      sql.add(' FROM [dbo].[TempCheck] T WITH(NOLOCK) 																						');
      sql.add(' INNER JOIN ControllerInfo C WITH(NOLOCK) ON C.SerialNo = T.SerialNo    														');
      sql.add(' LEFT JOIN (SELECT * FROM ClosestTemp WHERE rn = 1) CT ON CT.[No] = T.[No]														');
      sql.add(' WHERE C.ControllerID = '+DetailControllerID.ToString);
      SQL.Add('and t.CheckDate >= '+#39+FormatDateTime('yyyy-mm-dd',DateEditGraph1.Date)
                                 +FormatDateTime(' hh:nn:ss',TimeEditGraph1.Time)+#39);
      SQL.Add('and t.CheckDate <= '+#39+FormatDateTime('yyyy-mm-dd',DateEditGraph2.Date)
                                 +FormatDateTime(' hh:nn:ss',TimeEditGraph2.Time)+#39);
      SQL.Add('Order by t.CheckDate');
      open;
      ...
      begin
        ...
        StringGrid2.Cells[StringColumn8.Index,i]  := FieldByName('ErrCode').AsString;
        StringGrid2.Cells[StringColumn9.Index,i]  := FieldByName('StartTime').AsString;
        StringGrid2.Cells[StringColumn10.Index,i] := FieldByName('EndTime').AsString;

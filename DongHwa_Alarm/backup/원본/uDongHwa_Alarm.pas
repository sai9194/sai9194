...
function Cooldown: TDateTime; inline;
begin
//  Result := EncodeTime(0, COOLDOWN_MIN, 0, 0);
end;
...
begin
  try
    ...
    begin
      ...
      FDQuery3.ExecSQL;
      Memo1.Lines.Add(FormatDateTime('yyyy-mm-dd hh:nn:ss',Now) +' : '+  FDQuery3.SQL.Text );
    end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        sql.add(' 	,Err																                                	');
        sql.add(' 	,CheckDate 	, MU.Name												                          ');
        sql.add(' 	from(select Temp ,PowerState, Err                                    	');
        sql.add(' 			,CheckDate, ControllerID from TempState                          	');
        sql.add(' 			Where CheckDate is not null)as TS				                          ');
        sql.add(' 	Inner Join ControllerInfo as C							                          ');
        sql.add(' 	On TS.ControllerID = C.ControllerID					                          ');
        sql.add(' 	inner join  (SELECT LOGINID,UserID, MasterID, Name FROM MasterUser)  as MU	');
        sql.add(' 	on  MU.MasterID = C.MasterID and MU.UserID =C.MasterID 				      	');
        sql.add(' 	Where C.MasterID > 0												                        	');
        sql.add(' 	and CheckDate >= GETDATE() - 30												                '); // 30일이상 업데이트 안된것 제외
        //sql.add(' 	Where C.MasterID = 59												                        	'); TEST용
        sql.add(' 	Order By C.MasterID ,  C.ControllerID 											          ');
        Open;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  FDQuery2.ExecSQL;
                  Memo1.Lines.Add(FormatDateTime('yyyy-mm-dd hh:nn:ss',Now) +' : '+MName+  ' Update : '+UpdateNo);
                end;
              end;
...
begin
  ...
  begin
    try
      ...
      Memo1.Lines.Add(FormatDateTime('yyyy-mm-dd hh:nn:ss',Now) +' : '+ 'Reconnected');
    except
      on E: Exception do
        Memo1.Lines.Add('Reconnect failed: ' + E.Message);
    end;
  end;
  ...
  begin
    ...
    begin
      Timer1.Enabled := True;
      Memo1.Lines.Add(FormatDateTime('yyyy-mm-dd hh:nn:ss',Now) +' : '+ 'Main timer re-enabled');
    end;
  end;
end;
